<template>
  <div>
    ...
    <PresaleModal @register="registerModal" @success="handleSuccess" />
  </div>
</template>

<script lang="ts" name="mall-presale" setup>
  import { BasicTable, TableAction } from '/@/components/Table';
  import { useModal } from '/@/components/Modal';
  import PresaleModal from './components/Modal.vue';
  import { columns, searchFormSchema } from './presale.data';
  import { list, deletePresale, startActivity, endActivity } from './presale.api';
  ...
  function handleDelete(record: any) { deletePresale({ id: record.id }, reload); }
  function handleStart(record: any) {
    Modal.confirm({
      title: '确认开始', content: '确定要开始该预售活动吗？',
      onOk: () => startActivity({ id: record.id }).then(() => reload()),
    });
  }
  function handleEnd(record: any) {
    Modal.confirm({
      title: '确认结束', content: '确定要结束该预售活动吗？',
