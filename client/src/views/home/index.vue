...
const displayRecommendations = computed(() => {
  if (activeChannel.value === 'course') return mockRecommendations.filter(r => r.type === 'course')
  if (activeChannel.value === 'live') return mockRecommendations.filter(r => r.type === 'live')
...
async function loadCategories() {
  try {
    ...
    if (Array.isArray(tree) && tree.length > 0) { /* 保留 mock 为主 */ }
  } catch { /* 兜底 mock */ }
}
...
watch(activeChannel, (ch) => {
  if (ch === 'mall' && products.value.length === 0) loadProducts()
})
