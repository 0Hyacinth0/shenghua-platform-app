    ...
    <a-spin :spinning="loading">
      ...
      <a-table v-else :data-source="list" :pagination="false" rowKey="id" size="small" bordered>
        ...
        <a-table-column title="价格" width="120">
          <template #default="{ record }"><span style="color:var(--color-primary, #FF4D4F)">￥{{ record.minPrice }}</span></template>
        </a-table-column>
...
<style scoped>
.merchant-products-page { background: var(--color-bg-page, #F7F7F8); padding: 16px; min-height: 100vh; padding-bottom: calc(var(--tab-bar-height, 56px) + var(--safe-bottom, 0px) + 12px); }
